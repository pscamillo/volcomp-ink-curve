...
import os
import json, json, numpy as np, zarr, tifffile
D = os.path.expanduser('~/challenges/vesuvius/ink-lens/metric/volcomp'); C = os.path.expanduser('~/challenges/vesuvius/ink-lens/metric/destila/segments_C/pherc0139-w043')
...
for q in (2, 4, 16):
    if os.path.exists(f'{D}/preds_q{q}_novo.tif'):
        r, d = medida(f'{D}/preds_q{q}_novo.tif'); pts[q] = (r, d, cod[f'q{q}']['taxa'])
r0, d0, _ = pts[1]
