...
ROOT = os.path.expanduser('~/challenges/vesuvius/villa_ink/ink-detection'); D = os.path.expanduser('~/challenges/vesuvius/ink-lens/metric/volcomp_w035')
...
ro = Z(f'{D}/w043_orig.zarr'); rv = Z(f'{D}/w043_vc.zarr'); off = Z(f'{ROOT}/ink-dataset/pherc0139_9um/w035/w035.zarr')
...
r_off, kk, dy, dx = best
print(f'gate: render original nosso (fatia {k}) vs w035.zarr oficial fatia {kk}, offset ({dy},{dx}): r = {r_off:.4f} (exigido ≥ 0,95)')
...
# predições
ink = Z(f'{ROOT}/labels_hf/native9-w035/w035_inklabels.zarr')[14] > 0; sup = np.ones_like(ink)  # native9 sem supervision_mask: toda a região com dado
rng = np.random.default_rng(20260904)
print(f"{'braco':6} {'sent':4} {'razao':>7} {'d prima':>8} {'nulo':>6} {'n':>8}")
for b in ('orig', 'vc'):
    for s in ('', '_reverse'):
        a = tifffile.imread(f'{D}/preds_{b}{s}.tif').astype(np.float32)
...
print('  ref oficial (w035.zarr, fwd): 2,884 / 2,824.  criterio: |delta razao| <= 0,05 e |delta d| <= 0,10')
